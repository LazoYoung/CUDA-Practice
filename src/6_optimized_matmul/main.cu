...
__global__ void computeFromBlock(Matrix A, Matrix B, Matrix C) { // NOLINT(*-unnecessary-value-param)
    ...
    for (int step = 0; step < A.cols / blockSize; ++step) {
        ...
        for (int i = 0; i < blockSize; ++i) {
            float mul = __fmul_rn(shared_A.get(row, i), shared_B.get(i, col));
...
Result computeFromDevice(const MatrixSet &m, int blockSize, bool optimized) {
    assert(m.A.cols == m.B.rows);

    Result result(optimized ? "Parallel CUDA (optimized)" : "Parallel CUDA");
    ...
    cudaFuncSetAttribute(computeFromBlockOptimized, cudaFuncAttributeMaxDynamicSharedMemorySize, SHARED_BYTES);
    cudaFuncSetAttribute(computeFromBlock, cudaFuncAttributeMaxDynamicSharedMemorySize, SHARED_BYTES);

    result.timer.reset();

    if (optimized) {
        computeFromBlockOptimized<<<grid, block, sharedMem * 2>>>(d_A, d_B, d_C);
    } else {
...
int main() {
    ...
    measure(computeFromDevice, m2, blockSize, false);
    C2.print("C: ");
    compare(C1, C2);

    measure(computeFromDevice, m3, blockSize, true);
    C3.print("C: ");
    compare(C1, C3);
